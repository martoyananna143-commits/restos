...
async def send_document(
    ...
    :param caption: Optional caption for document.
    :param disable_notification: disable notification or not.
    :return: success.
    """
    try:
        ...
        return await send_document(
            bot, user_id, document_path, caption, disable_notification
        )  # Recursive call
    except exceptions.TelegramAPIError:
        logging.exception(f"Target [ID:{user_id}]: failed")
    else:
        logging.info(f"Target [ID:{user_id}]: document sent successfully")
        return True
    return False
