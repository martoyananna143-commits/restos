        ...
        if object_type in ("employees", "employee"):
            ...
            for eval_data in evaluations_data:
                ...
                if eval_id and eval_id not in seen_ids:
                    ...
                    date_str = eval_date.strftime('%Y-%m-%d %H:%M') if eval_date else 'N/A'
                    objects_list.append({
                        "id": eval_id,
                        "name": f"Замер #{eval_id}",
                        "type": "evaluation",
                        "display_text": f"📊 Замер #{eval_id}\n   Дата: {date_str} | Балл: {score:.1f}%",
                    })
    ...
    try:
        ...
        if object_type in ("employees", "employee"):
            ...
            if scores:
                ...
                detail_lines.append(f"Средний балл: {avg_score:.1f}%")
                detail_lines.append(f"Лучший результат: {max(scores):.1f}%")
                detail_lines.append(f"Худший результат: {min(scores):.1f}%")
            
            if last_eval_date:
                detail_lines.append(f"Последний замер: {last_eval_date.strftime('%Y-%m-%d %H:%M')}")
            ...
            if criterion_values:
                ...
                criterion_ids = [cv.get("criterion_id") for cv in criterion_values if cv.get("criterion_id")]
                criteria_map = {}
                if criterion_ids:
                ...
                for cv in criterion_values[:10]:  # Показываем первые 10
                    ...
                    criterion_name = criteria_map.get(criterion_id, f"ID {criterion_id}")
                    value = cv.get("value")
            ...
            all_criterion_sets = await criterion_set_service.get_by_organization_id(organization_id)
            sets_with_criterion = []
            for cs in all_criterion_sets:
                cs_full = await criterion_set_service.get_by_id(cs.id)
                if cs_full and hasattr(cs_full, 'criterion_ids') and cs_full.criterion_ids and object_id in cs_full.criterion_ids:
            ...
            criterion_ids = criterion_set.criterion_ids if hasattr(criterion_set, 'criterion_ids') else []
            if criterion_ids:
            ...
            for eval_data in evaluations_data:
                ...
                if eval_id and eval_id not in seen_eval_ids and eval_criterion_set_id == object_id:
                    seen_eval_ids.add(eval_id)
                    usage_count += 1
