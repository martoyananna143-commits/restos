...
class AssessmentAssignment(Base, TimestampMixin):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            name="fk_assessment_assignments_venue_company",
            ondelete="RESTRICT",
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            ...
            name="ck_assessment_assignments_purpose",
        ),
        CheckConstraint(
            "purpose != 'operational_walkthrough' OR venue_id IS NOT NULL",
            name="ck_assessment_assignments_operational_venue",
        ),
        CheckConstraint(
            "due_at IS NULL OR due_at > assigned_at",
            name="ck_assessment_assignments_due_after_assigned",
        ),
        CheckConstraint(
        ...
        Index(
            "ix_assessment_assignments_employee_status", "employee_profile_id", "status"
        ),
        Index("ix_assessment_assignments_company_status", "company_id", "status"),
        Index("ix_assessment_assignments_venue_status", "venue_id", "status"),
    ...
    assigned_by_employee_profile_id: Mapped[Optional[UUID]] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("employee_profiles.id", ondelete="SET NULL"),
        nullable=True,
    )
    assigned_by_account_id: Mapped[Optional[UUID]] = mapped_column(
        PGUUID(as_uuid=True),
...
class AssessmentAttempt(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('draft', 'submitted')", name="ck_assessment_attempts_status"
        ),
    ...
    ui_metadata_json: Mapped[dict[str, Any]] = mapped_column(
        JSONB,
        nullable=False,
        default=dict,
...
class AssessmentAttemptAnswer(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "attempt_id", "item_id", name="uq_assessment_attempt_answers_attempt_item"
        ),
        CheckConstraint(
    ...
    item_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("assessment_template_items.id", ondelete="RESTRICT"),
        nullable=False,
    )
