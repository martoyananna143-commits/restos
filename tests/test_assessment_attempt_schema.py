...
def test_assignment_schema_has_safe_foreign_keys_and_state_checks():
    ...
    assert {fk.ondelete for fk in table.foreign_key_constraints} == {"RESTRICT", "SET NULL"}
    checks = {value.name for value in table.constraints if isinstance(value, CheckConstraint)}
    assert checks == {
...
def test_attempt_and_answer_invariants():
    ...
    assert any(
        isinstance(value, UniqueConstraint) and value.name == "uq_assessment_attempts_assignment_id"
        for value in attempt.constraints
    )
    assert any(
        isinstance(value, UniqueConstraint) and value.name == "uq_assessment_attempt_answers_attempt_item"
        for value in answer.constraints
    )
    forbidden = {"token", "secret", "credential", "private_key", "score", "passing_value"}
...
def test_revision_and_json_defaults_are_server_controlled():
    ...
    assert not attempt.c.result_json.nullable is False
